...
- `app/main.py` — app entry: registers routers, CORS, static mounts. **It calls `Base.metadata.create_all()` on startup**, so new tables/columns can appear without a migration. Alembic migrations live in `backend/alembic/versions/` but the runtime does not auto-run them; schema in dev is effectively driven by `create_all` plus the ad-hoc `/fix_*` and `/migrate_*` GET endpoints defined at the bottom of `main.py`. Treat those endpoints as disposable production-patch hacks, not a pattern to extend.
